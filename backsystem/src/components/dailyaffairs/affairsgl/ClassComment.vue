<template>
    <div id="ClassComment">
        <el-row class="margin-bottom15">
            ...
            <el-col :span="12">
                <el-button class="float-r" type="danger" size="small" icon="el-icon-more-outline" @click="FunOpencomment">点评设置</el-button>
            </el-col>
        </el-row>
        <el-row class="margin-bottom15">
            <el-col :span="24">
        ...
        <el-row class="margin-bottom15">
            <el-col :span="24">
                <el-table :data="tableData" row-key="id" fit highlight-current-row style="width: 100%">
                    <el-table-column label="上课时间" width="450">
                        <template slot-scope="{row}">
                            <el-button size="mini" type="text">{{ row.name }}</el-button>
                        </template>
                    </el-table-column>
                    <el-table-column label="班级" width="450">
                        <template slot-scope="{row}">
                            <span>{{ row.name }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="课次" width="180">
                        <template slot-scope="{row}">
                            <span>{{ row.name }}</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="点评人数" width="180">
                        <template slot-scope="{row}">
                            <span>{{ row.name }}</span>
                        </template>
                    </el-table-column>
        ...
        <commentset ref="commentset"></commentset>
    </div>
</template>
<script>
    import {mapState,mapGetters,mapMutations,mapActions} from 'vuex'
    import commentset from '@/dialog/dailyaffairs/affairsgl/commentset'
    export default{
        name:'ClassComment',
        components:{
            commentset
        ...
        methods:{
            FunOpencomment(){
                let child = this.$refs.commentset
                child.dialogCommentset.isOpen = true
            }
        }
    }
</script>
